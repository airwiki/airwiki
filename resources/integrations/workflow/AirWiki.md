...
If a project manifest exists but the current task does not expose `open_airwiki_project` or `search_airwiki_memory`, the task loaded an older AirWiki integration. Do not fall back to personal memory or recreate `.airwiki`. Tell the user to refresh or update the client in **AirWiki → Settings → AI apps**, then start a new task or conversation in the same project; restart the client only if the tools are still missing.
