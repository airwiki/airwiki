...
| Typed-evidence ceiling protocol v2 | Frozen preregistration [#32](https://github.com/airwiki/airwiki/pull/32), 2026-07-19 | **Rejected before scoring** | The first and only hosted source-draft attempt completed, but the frozen validator rejected its claims because they were not in canonical text/relation order. No annotation artifact, adjudication, execution manifest or score was accepted, and the protocol prohibits a retry. Retire the one-off runner and do not treat this attempt as evidence for or against typed retrieval quality; it only rejects this annotation protocol. |
| Bounded missing-fact consumer policy | Preregistered offline calibration, 2026-07-19 | **Rejected in calibration** | Baseline and candidate both recovered 3/5 expected groups (0.60), with factual and citation precision of 1.00 and absence abstention of 1.00. Both missed the two Brio groups because every returned packet was empty; the candidate recovered no group missed by baseline and also exceeded its call and tool-contract gates. No holdout model run was executed. Retire the runner and fixture, keep MCP instructions unchanged and do not treat consumer prompting as a way to recover evidence filtered out at the source. |
| Typed-evidence coverage protocol v3 | [Frozen local contract](typed-evidence-coverage-v3.md) tracked by [#57](https://github.com/airwiki/airwiki/issues/57) | **Preregistration in review; unobserved** | Order is canonicalized rather than treated as semantics; source and question extraction remain isolated; every semantic field is bound to exact UTF-8 spans; blind human gold, two deterministic platform runs and causal controls are required. No development or promotion package has been created or scored. A reviewed contract only authorizes fresh data collection, not a product change. |
