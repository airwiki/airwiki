...
| Public queries exhaust an owner | Three-index, 64-candidate, 12-peer and two-collection caps; bounded payloads, semaphores, rate limits, a 1 s index stage and a fresh 800 ms owner stage | Bounded local verification and ranking add work between the two network stages; distributed abuse can still consume bounded relay and inference capacity |
