...
| Retrieval-stage attribution | [Active evaluator](retrieval-quality-evaluation.md#v3-corrected-contract-observation) | **Accepted diagnostic** | Source-candidate Recall@10 is 1.00 for all 18 expected groups in active schema v3. mMARCO retains 13 groups, rejects five and accepts two non-answering fragments; no expected group is lost in candidate generation, top-k truncation or revalidation. Keep stage attribution in the evaluator, not the product path. |
