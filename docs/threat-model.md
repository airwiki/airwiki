...
| Public registrations exhaust an index | Manifest remaining and signed lifetimes are at most 24 hours with bounded future skew; each SQLite catalog admits at most 100,000 total rows and 1,000 per publisher; expired manifests are purged before admission; compact tombstone high-water marks remain for the node lifetime and count against both ceilings | An attacker can consume the bounded shared capacity and deny new registrations; operators monitor sanitized database and disk consumption and may replace or retire the index |
