...
| Public registrations exhaust an index | Manifests expire within 24 hours of receipt; each SQLite catalog admits at most 100,000 total rows and 1,000 per publisher; expired manifests and tombstones are purged before admission; tombstones remain for the maximum replay window | An attacker can consume the bounded shared capacity and deny new registrations until expiry; operators monitor sanitized database and disk consumption and may replace or retire the index |
