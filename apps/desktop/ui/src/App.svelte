<script lang="ts">
  ...
  function openServiceStatus(target: ServiceTarget) {
    ...
    destination = 'wikis';
    selectedWikiId = null;
    connectionsOpen = target === 'lan' || target === 'mcp';
    pushHash('#wikis');
    scrollMainTo(0);
    void runConnectivityAction('refresh');
    if (target === 'mcp') void runIntegrationAction({ kind: 'refresh' });
    if (target === 'public') requestAnimationFrame(() => document.querySelector('#public-wikis')?.scrollIntoView({ block: 'start' }));
    if (target === 'indexing') void refreshHealth();
  }
  ...
  async function openSearchHit(hit: SearchHitSummary) {
    ...
    if (localWiki && hit.nodeId === snapshot?.nodeId) {
      ...
      await loadWikiPage(hit.wikiId, { kind: 'concept', id: hit.conceptId });
      return;
    }
    try {
        ...
        <div class="route-page drive-route" data-route={destination}>
          ...
          {:else if destination === 'wikis' && !selectedWiki}
            ...
            <section class="workspace-section public-discovery" id="public-wikis" aria-labelledby="public-wikis-title">
              ...
              {#if snapshot?.search?.hits.some((hit) => hit.nodeId !== snapshot?.nodeId)}<div class="search-results">{#each snapshot.search.hits.filter((hit) => hit.nodeId !== snapshot?.nodeId) as hit (`${hit.nodeId}:${hit.wikiId}:${hit.conceptId}:${hit.rank}`)}<article><small>{wikiNameFor(hit.wikiId)}</small><h3>{hit.title}</h3><p>{hit.snippet}</p><button class="text-action" onclick={() => openSearchHit(hit)}>{t('action-open')}</button></article>{/each}</div>{:else}<div class="inline-empty"><BookOpen size={20} aria-hidden="true" /><span><strong>{t('desktop-public-empty-title')}</strong><small>{t('desktop-public-search-help')}</small></span></div>{/if}
                ...
                <div class="file-browser">
                  <aside class="file-list" aria-label={t('knowledge-pages')}>
                    {#if snapshot.knowledge?.wikiId === selectedWiki.id}
                      ...
                      {#each snapshot.knowledge.concepts as concept (pageKey(concept.page))}<button onclick={() => openKnowledgePage(concept.page)}><FileText size={17} aria-hidden="true" /><span><strong>{concept.title}</strong><small>{concept.description}</small></span></button>{/each}
                    {/if}
                  </aside>
          ...
          {:else if destination === 'shared'}
            ...
            {:else}
              <section class="public-discovery"><header><h2>{t('desktop-public-discover-title')}</h2><p>{t('desktop-public-discover-body')}</p></header><p>{t('desktop-public-search-help')}</p>{#if snapshot?.search?.hits.length}<div class="search-results">{#each snapshot!.search!.hits.filter((hit) => hit.nodeId !== snapshot?.nodeId) as hit (`${hit.nodeId}:${hit.wikiId}:${hit.conceptId}:${hit.rank}`)}<article><small>{wikiNameFor(hit.wikiId)}</small><h3>{hit.title}</h3><p>{hit.snippet}</p><button class="text-action" onclick={() => openSearchHit(hit)}>{t('action-open')}</button></article>{/each}</div>{/if}{#if snapshot.publicBrowse}<div class="public-browse-detail"><div class="section-heading"><div><h3>{snapshot.publicBrowse.wikiName ?? t('desktop-public-origin-missing')}</h3><p>{snapshot.publicBrowse.description ?? ''}</p></div>{#if snapshot.publicBrowse.publisherId}<button class="danger" onclick={() => changePublisherBlock(snapshot!.publicBrowse!.publisherId!, true)}>{t('search-public-block-publisher')}</button>{/if}</div>{#each snapshot.publicBrowse.concepts as concept (`${concept.conceptId}:${concept.sourceRevision}`)}<article><small>{concept.conceptType} · {concept.language}</small><h3>{concept.title}</h3><p>{concept.summary}</p></article>{/each}{#if snapshot.publicBrowse.nextCursor}<button class="secondary" onclick={loadMorePublicConcepts}>{t('search-public-browse-more')}</button>{/if}</div>{/if}</section>
            {/if}
          {:else if destination === 'search'}
            <header class="page-heading"><div><h1>{t('desktop-page-search-title')}</h1><p>{t('desktop-page-search-body')}</p></div></header>
            {#if snapshot.search}<div class="search-results" aria-live="polite">{#if snapshot.search.status === 'searching'}<div class="search-state working" role="status"><span class="status-dot working" aria-hidden="true"></span><span>{t('search-running')}</span></div>{:else if snapshot.search.status === 'failed'}<div class="search-state error" role="alert"><AlertTriangle size={17} aria-hidden="true" /><span>{t('search-error-title')}</span></div>{:else if snapshot.search.hits.length > 0}<p class="section-label">{t('desktop-search-found')}</p>{/if}{#each snapshot.search.hits as hit (`${hit.nodeId}:${hit.wikiId}:${hit.conceptId}:${hit.rank}`)}<article><small>{wikiNameFor(hit.wikiId)} · {hit.headingOrPage}</small><h3>{hit.title}</h3><p>{hit.snippet}</p><div class="citation-row"><span>{hit.nodeId === snapshot.nodeId ? t('desktop-search-local-source') : t('desktop-public-network')}</span><span>{t('search-revision', { revision: hit.sourceRevision })}</span></div><button class="text-action" onclick={() => openSearchHit(hit)} disabled={publicBrowseRequestId !== null}>{t('action-open')}</button></article>{:else}{#if snapshot.search.status === 'complete'}<div class="table-empty"><strong>{t('search-empty-title')}</strong><p>{t('search-empty-body')}</p></div>{/if}{/each}</div>{:else}<div class="search-welcome"><BookOpen size={32} aria-hidden="true" /><h2>{t('desktop-search-welcome-title')}</h2><p>{t('desktop-search-welcome-body')}</p></div>{/if}
          {:else if destination === 'system'}
            ...
            <div class="settings-page">
              ...
              {#if systemSection === 'preferences'}<section id="system-preferences"><p class="section-label">{t('desktop-preferences')}</p><h2>{t('desktop-preferences')}</h2><div class="settings-form"><label><span>{t('settings-language')}</span><select bind:value={locale}><option value="system">{t('language-system')}</option><option value="en">English</option><option value="es">Español</option></select></label><label><span>{t('settings-theme')}</span><select bind:value={theme}><option value="system">{t('theme-system')}</option><option value="light">{t('theme-light')}</option><option value="dark">{t('theme-dark')}</option></select></label><label><span>{t('desktop-lan')}</span><select bind:value={lanPreference}><option value="undecided">{t('settings-lan-undecided')}</option><option value="disabled">{t('onboarding-lan-disable')}</option><option value="enabled">{t('onboarding-lan-enable')}</option></select></label><label><span>{t('desktop-close')}</span><select bind:value={closeBehavior}><option value="ask">{t('desktop-ask')}</option><option value="hide">{t('desktop-hide-tray')}</option><option value="quit">{t('desktop-quit')}</option></select></label><label class="check"><input type="checkbox" bind:checked={automaticUpdateChecks} />{t('updates-automatic')}</label><button class="primary" onclick={() => savePreferences()} disabled={actionBusy}>{t('desktop-save-preferences')}</button></div></section><section><p class="section-label">{t('settings-login-title')}</p><h2>{t('settings-login-heading')}</h2><p>{autostartLabel(locale)}</p><div class="row-actions"><button class="secondary" onclick={() => changeAutostart(true)} disabled={autostartBusy}>{t('action-enable')}</button><button class="text-action" onclick={refreshAutostartState} disabled={autostartBusy}>{t('action-refresh')}</button></div></section>{/if}
