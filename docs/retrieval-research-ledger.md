...
| Reviewed claim identity in the existing mMARCO passage | Preregistered local probe, 2026-07-18 | **Planned** | Keep the current candidate batches and mMARCO artifact, but prepend each human-reviewed claim's subject, relation, scope, temporal and polarity fields to its passage. Continue only if the observed v2 diagnostic recovers at least 13/18 expected groups, emits at most 2 unexpected facts and preserves zero forbidden, provenance, duplicate, mapping and stability errors. This is a feasibility gate, not promotion evidence; a passing result still requires fresh domains. |
