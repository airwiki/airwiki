...
| mMARCO text-only passage ablation | Active bounded experiment | **Proposed** | Keep the model, query, candidate pool and order fixed, but classify only the visible snippet instead of repeating title and heading metadata. Continue only if v2 strictly reduces both the six expected groups rejected by the mask and the three unexpected survivors, with zero forbidden evidence or audit, provenance, deduplication, revalidation and stability errors. V2 is diagnostic only; promotion still requires the predeclared fresh-domain gate on both platforms. |
